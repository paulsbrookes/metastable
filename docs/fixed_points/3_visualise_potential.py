...
x_c = np.linspace(-20, 20, 50)
p_c = np.linspace(-20, 20, 50)
...
# Apply nonlinear scaling to compress the dynamic range
# Using sqrt to make smaller arrows more visible while preserving direction
V_x = V_x * np.sqrt(velocities_magnitude) / velocities_magnitude
V_p = V_p * np.sqrt(velocities_magnitude) / velocities_magnitude

# Scale to maintain maximum arrow length at 2 * grid_spacing
scale_factor = 4 * grid_spacing / np.max(np.sqrt(V_x**2 + V_p**2))
