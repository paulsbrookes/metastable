...
if __name__ == "__main__":
    ...
    jacobian = eom.jacobian_func(fixed_point)

    eigenvalues, eigenvectors = np.linalg.eig(jacobian)


    eigenvalues_alt = fixed_point_map.eigenvalues[epsilon_idx, kappa_idx, fixed_point_type]

    # eigenvectors is a 4x4 array where the first index is the component and the second index is the eigenvalue
    eigenvectors_alt = fixed_point_map.eigenvectors[epsilon_idx, kappa_idx, fixed_point_type]

    assert np.allclose(eigenvalues, eigenvalues_alt)
    assert np.allclose(eigenvectors, eigenvectors_alt)
