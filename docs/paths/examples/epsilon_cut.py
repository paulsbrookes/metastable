...
if __name__ == "__main__":
    ...
    epsilon_sweeps = generate_sweep_index_pairs(epsilon_boundaries, sweep_fraction=0.2)
    ...
    if epsilon_sweeps.bright_saddle and epsilon_sweeps.dim_saddle:
        # Extract kappa and epsilon values for the bright_saddle cut
        bright_cut_kappas = [fixed_point_map.kappa_linspace[pair.kappa_idx] for pair in epsilon_sweeps.bright_saddle]
        bright_cut_epsilons = [fixed_point_map.epsilon_linspace[pair.epsilon_idx] for pair in epsilon_sweeps.bright_saddle]
        
        # Extract kappa and epsilon values for the dim_saddle cut
        dim_cut_kappas = [fixed_point_map.kappa_linspace[pair.kappa_idx] for pair in epsilon_sweeps.dim_saddle]
        dim_cut_epsilons = [fixed_point_map.epsilon_linspace[pair.epsilon_idx] for pair in epsilon_sweeps.dim_saddle]
