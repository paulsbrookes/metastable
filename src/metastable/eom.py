...
class EOM:
    ...
    epsilon_sym, delta_sym, chi_sym, kappa_sym = sympy.symbols(
        "epsilon delta chi kappa"
    )
    ...
    H_sym = (
        ...
        - (chi_sym / 2) * (xc**2 + pc**2 - xq**2 - pq**2) * (xc * pq - pc * xq)
        + 2 * epsilon_sym * xq
    )
    ...
    y_dot_classical_expr = sympy.Matrix(y_dot_expr.subs([(xq, 0), (pq, 0)])[0:2])
    ...
    def __init__(self, params: Params):
        ...
        self.y_dot_func = lambda y: sympy.lambdify(self.sym, self.y_dot_expr_sub)(*y)[
            :, 0
        ]
        self.jacobian_func = lambda y: sympy.lambdify(self.sym, self.jacobian_expr_sub)(
            *y
        )
