...
def fixed_point_tracker_duffing(
    params_frame,
    alpha0=0,
    fill_value=None,
    threshold=1e-4,
    crosscheck_frame=None,
    columns=["a"],
):
    trip = False
    ...
    for i, idx in tqdm(enumerate(index)):
        ...
        if not trip:
            alpha_fixed = locate_fixed_point_mf_duffing(
                params, alpha0=[alpha0.real, alpha0.imag]
            )
    ...
    amplitude_frame = pd.DataFrame(
        amplitude_array, index=params_frame.index, columns=columns
    )
...
def mf_characterise_duffing(params_frame):
    ...
    mf_amplitude_frame_bright = fixed_point_tracker_duffing(
        reversed_params_frame, alpha0=alpha0
    )
    mf_amplitude_frame_dim = fixed_point_tracker_duffing(
        params_frame,
        alpha0=alpha0,
        columns=["a_dim"],
    ...
    mf_amplitude_frame = pd.concat(
        [mf_amplitude_frame_bright, mf_amplitude_frame_dim], axis=1
    )

    overlap_find = ~np.isclose(
        mf_amplitude_frame["a_bright"].values, mf_amplitude_frame["a_dim"].values
    )

    if not np.any(overlap_find):
        mf_amplitude_frame = pd.DataFrame(
            mf_amplitude_frame.values[:, 0],
            index=mf_amplitude_frame.index,
            columns=["a"],
        )
    else:
...
def dalpha_calc_mf_duffing(alpha, params):
    dalpha = (
        params.eps
        - 1j * (params.fc - params.fd) * alpha
        - 2 * 1j * params.chi * alpha * np.abs(alpha) ** 2
        - 0.5 * params.kappa * alpha
    )
    return dalpha
...
def ham_duffing_gen(params):
    ...
    H = (
        (params.fc - params.fd) * a.dag() * a
        + params.chi * a.dag() * a.dag() * a * a
        + 1j * params.eps * (a.dag() - a)
    )
    return H
...
def state_cut_gen(
    ...
    extend=0.001,
    iterations=2,
    prune_threshold=0.23,
    fd_mf_limits=[10.4, 10.5],
    backend="slepc",
    sigma=-0.001,
    k=2,
    n_frequencies=21,
    ...
    if fd_array is None:
        if results_all is None:
            ...
            fd_array = _new_frequencies_gen(
                results_all.index,
                np.abs(results_all["rate_ad"].values),
                threshold=threshold,
            )

    for i in range(iterations):
        print(
            "epsilon = %f, iteration = %i, number of frequencies = %i"
            % (params.eps, i, fd_array.shape[0])
        )
        for fd_idx, fd in tqdm(enumerate(fd_array)):
            ...
            try:
                rates, states = _eigensolver_wrapper(
                    L.data, backend=backend, sigma=sigma, k=k
                )
                ...
                results = pd.DataFrame(
                    results,
                    columns=["rate_ss", "rate_ad", "state_ss", "state_ad"],
                    index=[params.fd],
                )
                if results_all is None:
                    results_all = results
...
def _new_frequencies_gen(x, y, threshold=10.0):
    ...
    midpoint_curvatures = np.concatenate(
        (np.array([abs_curvatures[0]]), mean_curvatures)
    )
    midpoint_curvatures = np.concatenate(
        (midpoint_curvatures, np.array([abs_curvatures[n_points - 3]]))
    )
    ...
    intervals = np.diff(x)
    num_of_sections_required = np.ceil(
        intervals * np.sqrt(midpoint_curvatures_normed / threshold)
    ).astype(int)
    new_fd_points = np.array([])
    for index in np.arange(n_points - 1):
        multi_section = np.linspace(
            x[index], x[index + 1], num_of_sections_required[index] + 1
        )
...
def liouvillian_eigenstate_sweep(
    params, sweep_param_name, sweep_param_values, k=2, backend="scipy", sigma=-0.001
):
    columns = ["rate_" + str(n) for n in range(k)] + [
        "state_" + str(n) for n in range(k)
    ]
    content = []
...
def calc_metastable_states(
    rho_ss, rho_ad, x_limits=[-2, 2], n_x_points=21, offset=1e-10
):
    ...
    if x_metastable_estimates.shape[0] != 2:
        ...
        packaged_results = pd.DataFrame(
            [[None, None, None, None]], columns=["rho_d", "rho_b", "p_d", "p_b"]
        )
    ...
    packaged_results = pd.DataFrame(
        [np.hstack((rho_array, occupations))], columns=["rho_d", "rho_b", "p_d", "p_b"]
    )
