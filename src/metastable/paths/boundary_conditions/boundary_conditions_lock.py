...
@dataclass
class BoundaryLockParams:
    """Parameters for controlling boundary condition locking behavior."""
    stable_threshold: float = 1e-2
    stable_linear_coefficient: float = 0.0
    saddle_threshold: float = 1e-2
    saddle_linear_coefficient: float = 0.0
...
def generate_boundary_condition_func(
    ...
    lock_params: BoundaryLockParams = BoundaryLockParams()
    ...
    def select_boundary_conditions(stable_components, saddle_components):
        ...
        # Check if magnitude of stable_components[2:4] is less than threshold
        stable_comp_magnitude = np.linalg.norm(stable_components[2:4])
        # Make stable_lock_condition increase linearly above the threshold
        stable_lock_condition = 0.0 if stable_comp_magnitude < lock_params.stable_threshold else (stable_comp_magnitude - lock_params.stable_threshold) * lock_params.stable_linear_coefficient
        
        # Check if magnitude of saddle_components[0:2] is less than saddle_threshold
        saddle_comp_magnitude = np.linalg.norm(saddle_components[0:2])
        # Make saddle_lock_condition increase linearly above the threshold
        saddle_lock_condition = 0.0 if saddle_comp_magnitude < lock_params.saddle_threshold else (saddle_comp_magnitude - lock_params.saddle_threshold) * lock_params.saddle_linear_coefficient
        
        # Combine the components for the boundary conditions
        return np.hstack([
            np.abs(stable_components[0]+stable_components[1]) + stable_lock_condition,
            np.abs(stable_components[0]-stable_components[1]) + stable_lock_condition,
            np.abs(saddle_components[2]) + saddle_lock_condition,
            np.abs(saddle_components[3]) + saddle_lock_condition,
            ])
